...
describe("onMessage", () => {
  ...
  describe("splitFragmentSeries false", () => {
    beforeAll(async () => {
      runner = new SmartFunctionsRunner({
        ...
        timeout: 1000000,
        params: {
          // Add any necessary parameters for the onMessage function here
          splitFragmentSeries: false,
        },
      });
    ...
    it("should process measurements with multiple fragments and series", async () => {
      const measurement = {
        payload: {
          id: "12345",
      ...
      const context = {
        linkedAsset: [
          assetTemperatureLink,
          assetTemperature2Link,
          assetPressureLink,
        ],
      };
      ...
      expect(
        debugLogs.some((log) =>
          log.args.some(
            (arg) => typeof arg === "string" && arg.includes("New measurement"),
          ),
        ),
      ).toBe(true);
    });
  ...
  describe("splitFragmentSeries true", () => {
    beforeAll(async () => {
      runner = new SmartFunctionsRunner({
        sourceFile: "onmessage.fn.ts",
        baseDir: path.resolve(__dirname),
    ...
    it("should return null and log debug message when linkedAsset is missing required fields", async () => {
      ...
      expect(
        debugLogs.some((log) =>
          log.args.some(
            (arg) =>
              typeof arg === "string" && arg.includes("Skipping linked asset"),
          ),
        ),
      ).toBe(true);
    });
